...
$qy->execute(array("pending"=>'pending',"delstatus"=>0));
               ...
               //echo json_encode($stmtPending);exit;
                ...
                $qyGiven=$conn->prepare("SELECT COUNT(losts.lost_id) AS total_given,sum(losts.ratio_postoffices) AS total_income FROM postoffices LEFT JOIN losts ON postoffices.postoff_id=losts.postoff_id  WHERE postoffices.postoff_id=:postoffid AND losts.status=:status AND postoffices.delete_status=:delstatus GROUP BY postoffices.postoff_id");
...
function changeAllChatStatus($datas){global $conn;
...
#
function changeIssuesStatus($datas){global $conn;
//update when delivered,seen
...
function addIssueChat($datas) { global $conn;
...
function getIssueChat($datas) { global $conn;
...
function searchIssueChat($key) {global $conn;
$data=null;
...
if($range["start"]!=null && $range["end"]!=null) {
	$start=$range["start"]." 00:00:01";$end=$range["end"]." 23:59:59";
		$prepWhere.=" AND losts.regdate>='$start' AND losts.regdate<='$end'";
	}
$sel=$qy.$prepWhere;
...
case 'GET':
  $urlData=json_encode($_GET);break;
  case'POST':
  $urlData=json_encode($_POST);break;
  break;
  default:
