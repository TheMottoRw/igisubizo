...
$server = new server;
$client = new client;
$lastPathArr = explode("\\", getcwd());
$lastPath = $lastPathArr[count($lastPathArr) - 1];
switch ($server->getHost()) {
    case 'localhost:12':
    case 'localhost':
        break;
    default:
        $lastPathArr1 = explode("/", $lastPath);
        $lastPath = $lastPathArr1[count($lastPathArr1) - 1];
        break;
}
//echo $lastPath."->".$server->getHost();exit;
if ($lastPath == 'ajax') {
...
function connect()
{
    $connection = null;
    try {
          $connection = new PDO("mysql:host=localhost;dbname=igisubizo", "super", "");
...
function updateUser($userid, $datas)
{
    ...
    try {
        ...
        $cate = safeInput($datas['category']);
        $address = safeInput($datas['address']);
        //check if Phone or nid or username or email exists to other

        $qryx = $conn->prepare("SELECT * FROM users WHERE uid!=:usrid AND (nid=:nid || phone=:phone || email=:email || username=:uname) AND delete_status=:delstatus");
        $qryx->execute(array("usrid" => $userid, "nid" => $nid, "phone" => $phone, "email" => $email, "uname" => $uname, "delstatus" => 0));
        if ($qryx->rowCount() == 0) {
            $qry = $conn->prepare("SELECT * FROM users WHERE uid=:usrid AND delete_status=:delstatus");

            $qry->execute(array("usrid" => $userid, "delstatus" => 0));
...
//echo $commissionAmount."-".$agaciroAmount."=".$remainAmount;exit;
    ...
    if ($count == 0) {
        try {
            ...
            if ($cnt == 1) {
                if ($commid != 0) {
                    //Notify Citizen that he is Registered

                    // $response=sendSMS(array("to"=>$phone,"IGISUBIZO","message"=>"Your Registration has successfull done"));
...
function getCitizensByCommissioner($datas)
{
    global $conn;
    $ft = null;
    $additionalWhere = "";
...
function updateCitizens($citizenid, $datas)
{
    $data = null;
    global $conn;
    global $spurl;
    ...
    try {
        $conn->beginTransaction();
        $qryx = $conn->prepare("SELECT * FROM citizens WHERE cit_id!=:citid AND (cit_phone=:phone || cit_nid=:nid)");
        $qryx->execute(array("citid" => $datas['id'], "phone" => $phone, "nid" => $nid));
        if ($qryx->rowCount() == 0) {
...
function getPostoffices($postoffid)
{
    global $conn;
    $data = null;
    $additionalWhere = "";
...
function getPostofficesReport($datas)
{
    global $conn;
    if (isset($datas['start']) && isset($datas['end'])) {
        $datas['start'] .= " 00:00:01";
...
function getLostsByPostoffices($datas)
{
    global $conn;
    $data = null;
...
function searchPostoffices($key)
{
    global $conn;
    $data = null;
...
function updatePostoffices($postoffid, $datas)
{
    $data = null;
    global $conn;
    $name = safeInput($datas['name']);
    ...
    $qyx = $conn->prepare("SELECT * FROM postoffices WHERE (name=:name || representative=:representative || phone=:phone) AND postoff_id!=:postoffid");
    $qyx->execute(array("name" => $name, "representative" => $representative, "phone" => $phone, "postoffid" => $postoffid));
    if ($qyx->rowCount() == 0) {
...
function requestResetPostoffices($postoffid, $datas)
{
    $data = null;
    global $conn;
    $phone = safeInput($datas['phone']);
...
function updatePostofficesPwd($postoffid, $datas)
{//for postoffice itself
    global $conn;
...
function resetPostoffices($resetid, $postoffid, $datas)
{
    $data = null;//for when password Forgotten
    global $conn;
...
function searchRepresenter($key)
{
    global $conn;
    $data = null;
...
function updateRepresenter($repid, $datas)
{
    $data = null;
    global $conn;
    $feed = null;
...
function getRepresenterByPost($postoffid)
{
    global $conn;
    $data = null;
...
function getQueue($datas)
{
    ...
    if (isset($datas['docid']) && is_numeric($datas['docid'])) {
        $additionalWhere .= " AND queue.queue_type=" . $datas['docid'];
    }
    if ($queueid == null) {
        if (isset($datas['start']) && isset($datas['end'])) {
            $datas['start'] .= " 00:00:01";
...
function updateQueue($queueid, $datas)
{
    $data = null;
    global $conn;
    ...
    // echo json_encode($datas)."-".searchType($datas['type']);exit;
    $docid = $docInfo["doc_id"];
    //avoid duplication
    $qry = $conn->prepare("SELECT * FROM queue 
                       WHERE queue_citizenid!=:citid AND (queue_type=:docid AND queue_identifier=:identifier) AND delete_status=:delstatus");
    ...
    if ($cnt == 0) {
      ...
      WHERE queue_id=:queueid");
            ...
            //echo json_encode($qy->errorInfo())+"=>".$ytpe;exit;
            if ($qy->rowCount() == 1) {
                $feed = "ok";
            } else {
                $feed = "fail";
...
//$distid=safeInput($geolocdt->distid);
    ...
    $qrie = $conn->prepare("SELECT * FROM losts 
                       WHERE owner=:name AND identifier=:identifier AND doctype_id=:docid AND delete_status=:delstatus AND status=:status");
    $qrie->execute(array("name" => $name, "identifier" => $identifier, "docid" => $docid, "status" => 'pending', "delstatus" => 0));
    $cnt = $qrie->rowCount();
    // echo $cnt;exit;
    if ($cnt > 0) {
        return $feed = "exist";
        ...
        if ($repid != 0 && $docid != null) {
            ...
            $lostId = $conn->lastInsertId();
            if ($qy) {

                //Get Queued User to Send Notification
                $qry = $conn->prepare("SELECT postoffices.postoff_id,postoffices.name as officename,postoffices.phone as officephone,queue.queue_id as queueid,citizens.cit_phone as queuephone,citizens.cit_names as ownername,doctypes.doctype FROM
     				 postoffices,queue,citizens,losts,doctypes WHERE postoffices.postoff_id=:postoffid AND postoffices.postoff_id=losts.postoff_id AND queue.queue_identifier=:identifier AND queue.queue_type=losts.doctype_id AND citizens.cit_names=losts.owner AND queue.queue_citizenid=citizens.cit_id AND doctypes.doc_id=losts.doctype_id AND queue.queue_type=doctypes.doc_id AND losts.lost_id=:lostsid AND losts.status=:status");
                $qry->execute(array("postoffid" => $postoffid, "identifier" => $identifier, "lostsid" => $lostId, "status" => 'pending'));
                if ($qry->rowCount() > 0) {
                    $ft = $qry->fetchAll(PDO::FETCH_ASSOC);
                    $doctype = $ft[0]["doctype"];
                    $officename = $ft[0]["officename"];
                    $officephone = $ft[0]["officephone"];
                    if (substr($officephone, 0, 1) == 0) {
                        $officephone = "25" . $ft[0]["officephone"];
                    } elseif (strlen($ft[0]["officephone"]) == 9) {
                        $officephone = "250" . $ft[0]["officephone"];
                    }
                    $queueid = $ft[0]["queueid"];
                    $number = $ft[0]["queuephone"];
                    $ownername = $ft[0]["ownername"];
...
*/    //FillMessage for History
                    $message = $ownername . " Muraho! Twishimiye kubamenyesha ko icyo mwabuze " . $doctype . " cyabonetse ubu mwagisanga kubiro byacu ";
                    $message1 = "Hi! Happy to inform  you that your lost item " . $doctype . " is now found,find it at our office:" . $officename . ",+" . $officephone;
//$response=sendAPIData($number,array("message"=>$message.">".$message1));

                    $date = date("Y-m-d H:i:s");
                    $qx = $conn->prepare("INSERT INTO notifications VALUES(:notid,:postoffid,:queueid,:message,:regdate)");
                    $qx->execute(array("notid" => '', "postoffid" => $postoffid, "queueid" => $ft[0]["queueid"], "message" => json_encode(array("message" => $message . "\n" . $message1, "response" => $response)), "regdate" => date("Y-m-d H:i:s")));
//echo json_encode($qx->errorInfo())."<br>";
...
//END Sending Notification
    ...
    $fop = fopen("regNot.hd", "a");
    fwrite($fop, date("Y-m-d H:i:s") . "  " . $feed . "\n");
    fclose($fop);
    return $feed;
}

function getLosts($lostsid)
{
    global $conn;
    $lostsid = $lostsid;
    $additionalWhere = "";
...
function searchLosts($key)
{
    ...
    $qy = $conn->prepare("SELECT postoffices.postoff_id,postoffices.name,representative.rep_name as representer,postoffices.phone,postoffices.address,provinces.province_name,districts.district_name,sector.name as sector_name,postoffices.cell,losts.lost_id,losts.owner,doctypes.doctype,losts.identifier,pay_id,pay_amount,losts.regdate 
					FROM losts INNER JOIN doctypes ON losts.doctype_id=doctypes.doc_id INNER JOIN postoffices ON  losts.postoff_id=postoffices.postoff_id INNER JOIN provinces ON postoffices.prov_id=provinces.id INNER JOIN districts ON provinces.id=districts.provinces_id AND postoffices.distr_id=districts.id  INNER JOIN sector ON districts.id=sector.district_id AND postoffices.sector=sector.id INNER JOIN representative  LEFT JOIN citizens ON citizens.cit_names=losts.owner AND citizens.reg_amount!=0 LEFT JOIN queue ON citizens.cit_id=queue.queue_citizenid AND losts.identifier=queue.queue_identifier AND losts.doctype_id=queue.queue_type AND doctypes.doc_id=queue.queue_type LEFT JOIN payment ON payment.pay_name='Losts' AND (CASE WHEN losts.regdate>=queue.regdate THEN payment.payer_type='Registered' ELSE payment.payer_type='Non Registered' END) WHERE losts.status=:pending AND losts.delete_status=:delstatus AND (losts.identifier=:key || losts.owner=:key)");
...
function searchLostsByKeys($postoffid, $key)
{
    global $conn;
    $data = null;
...
function updateLosts($lostsid, $datas)
{
    $data = null;
    global $conn;
    $name = safeInput($datas['owner']);
...
function updateType($docid, $datas)
{
    global $conn;
    $data = null;
    $doctype = safeInput($datas["doctype"]);
    $max = safeInput($datas["max"]);
    $qyx = $conn->prepare("SELECT * FROM doctypes WHERE doctype=:doctype AND doc_id!=:docid AND delete_status=:delstatus");
    $qyx->execute(array("doctype" => $doctype, "docid" => $docid, "delstatus" => 0));
...
function updatePayment($payid, $datas)
{
    global $conn;
    $data = null;
...
function updateCommission($id, $datas)
{
    global $conn;
    $data = null;
...
{
    global $conn;
    $qy = $conn->prepare("SELECT * FROM issues_chat WHERE isc_issueid=:issid AND isc_receiver_id=:toid AND isc_receiver_type=:totype AND (isc_status=:currstatus || isc_status=:curstatus)");
...
{
    global $conn;
    $qy = $conn->prepare("SELECT * FROM issues WHERE iss_status=:currstatus AND delete_status=:delstatus");
...
{
    global $conn;
    $qy = $conn->prepare("SELECT * FROM issues_chat WHERE iss_status=:currstatus AND delete_status=:delstatus ADN isc_receiver_id=:toid AND isc_receiver_type=:totype  GROUP BY isc_issueid");
...
//update when delivered,seen
    if ($datas['issid'] == 'all') {
        $qy = $conn->prepare("UPDATE issues_chat SET isc_status=:status WHERE isc_status=:currstatus AND isc_receiver_id=:toid AND isc_receiver_type=:totype");
        $qy->execute(array("status" => $datas['status'], "currstatus" => $datas['currstatus'], "toid" => $datas['toid'], "totype" => $datas['totype']));
    } else {
        $qy = $conn->prepare("UPDATE issues_chat SET isc_status=:status WHERE isc_issueid=:issid AND isc_status=:currstatus AND isc_receiver_id=:toid AND isc_receiver_type=:totype");
...
function changeIssuesStatus($datas)
{
    global $conn;
//update when delivered,seen
    if (!isset($datas['issid'])) {
        $qy = $conn->prepare("UPDATE issues SET iss_status=:status WHERE iss_status=:currstatus");
        $qy->execute(array("status" => $datas['status'], "currstatus" => $datas['currstatus']));
    } else {
        $qy = $conn->prepare("UPDATE issues SET iss_status=:status WHERE iss_status=:currstatus AND iss_id=:issid");
        $qy->execute(array("status" => $datas['status'], "currstatus" => $datas['currstatus'], "issid" => $datas['issid']));
    }
}

#==========SUPERADMIN==============
function changePwd($datas)
{
    global $conn;
    if (isset($datas['id'])) {
        $uid = $datas['id'];
...
function updateCategory($docid, $datas)
{
    global $conn;
    $data = null;
    $usrt_name = safeInput($datas["usertype"]);
...
function addWithdrawCompany($datas)
{
    global $conn;
    $name = safeInput($datas["name"]);
...
function getWithdrawCompany($wthcid)
{
    global $conn;
    $data = null;
    if ($wthcid == null) {
...
function searchWithdrawCompany($key)
{
    global $conn;
    $data = null;
...
function updateWithdrawCompany($wthcid, $datas)
{
    global $conn;
    $data = null;
    $name = safeInput($datas["name"]);
    $accr = safeInput($datas["accr"]);
    $qyx = $conn->prepare("SELECT * FROM withdraw_company WHERE wthc_name=:name || wthc_accronym=:accr AND wthc_id!=:wthcid AND delete_status=:delstatus");
    $qyx->execute(array("name" => $name, "accr" => $accr, "wthcid" => $wthcid, "delstatus" => 0));
    if ($qyx->rowCount() == 0) {
...
function updatePaymentMode($pmtdid, $datas)
{
    ...
    $accname = safeInput($datas["accname"]);
    $accnumber = safeInput($datas["accnumber"]);
    $qryx = $conn->prepare("SELECT * FROM payment_mode WHERE pmtd_name=:modename AND pmtd_company=:company AND pmtd_account_name=:accname AND pmtd_account_number=:accnumber AND pmtd_id!=:pmtdid");
    $qryx->execute(array("pmtdid" => $pmtdid, "modename" => $modename, "company" => $company, "accname" => $accname, "accnumber" => $accnumber));
    if ($qryx->rowCount() == 0) {
...
function addPaymentHistory($datas)
{
    ...
    $cnt = $qry->rowCount();
    if ($cnt == 0) {
        $qy = $conn->prepare("INSERT INTO payment_history VALUES(:pmthid,:payerid,:payertype,:mode,:sendername,:senderphone,:amount,:reason,:status,:approvedby,:approvaldate,:delstatus,:delreason,:regdate)");
        $qy->execute(array("pmthid" => '', "payerid" => $payerid, "payertype" => $payertype, "mode" => $mode, "sendername" => $datas['sendername'], "senderphone" => $datas['senderphone'], "amount" => $amount, "reason" => "", "status" => 'pending', "approvedby" => 0, "approvaldate" => "0000-00-00", "delstatus" => 0, "delreason" => '', "regdate" => $date));
        if ($qy) {
...
function declinePaymentHistory($datas)
{
    global $conn;
    $qyx = $conn->prepare("SELECT * FROM payment_history WHERE pmth_id=:pmthid AND pmth_status=:status");
    $qyx->execute(array("pmthid" => $datas['id'], "status" => 'pending'));
    if ($qyx->rowCount() > 0) {
        $payInfo = $qyx->fetchAll(PDO::FETCH_ASSOC);
//get payer informations for Notifications
        switch ($payInfo['pmth_payer_type']) {
            case 'Commissioner':
                $payerInfo = getUsers(array("id" => $payInfo['pmth_payer_id']))['users'][0];
                $phone = $payerInfo['phone'];
                $email = $payerInfo['email'];
                break;
            case 'Postoffice':
                $payerInfo = getPostoffices(array("id" => $payInfo['pmth_payer_id']))['postoffices'][0];
                $phone = $payerInfo['phone'];
                $email = $payerInfo['email'];
                break;
            case 'Citizen':
                $payerInfo = getCitizens(array("id" => $payInfo['pmth_payer_id']))['citizens'][0];
                $phone = $payerInfo['cit_phone'];
                $email = $payerInfo['cit_email'];
                break;
            default:
                break;
        }
        $qy = $conn->prepare("UPDATE payment_history SET pmth_status=:status,pmth_reason=:reason,pmth_approvedby=:sessid,pmth_approval_date=:dates WHERE  pmth_id=:pmthid");
        $qy->execute(array("pmthid" => $datas['id'], "reason" => $datas['reason'], "status" => 'declined', "sessid" => $datas['sessid'], "dates" => date("Y-m-d H:i:s")));
        if ($qy) {
            //Notify Payer either by SMS or Email
            $message = "Your payment done on " . $payInfo['regdate'] . " with amount of " . $payInfo['pmth_amount'] . " RWF has been declined <br> Reason:" . $datas['reason'] . "<br> Today " . $date;
            //    $response= sendEmail(array("from"=>'noreply@bfg.rw',"fromName"=>'IGISUBIZO System',"to"=>$email,"replyTo"=>'noreply',"subject"=>"Payment Declined","message"=>$message));
            //Using SMS

            //$response=sendSMS(array("to"=>$phone,"sender"=>'IGISUBIZO',"messag"=>$message));
            $feed = "ok";
        } else {
            $feed = "fail";
        }
...
function addWithdrawAccounts($datas)
{
    global $conn;
...
function getWithdrawAccounts($wthacid)
{
    global $conn;
    $data = null;
    $additionalWhere = "";
...
function searchWithdrawAccounts($key)
{
    global $conn;
    $data = null;
...
function updateWithdrawAccounts($wthacid, $datas)
{
    global $conn;
    $data = null;
    $accownerid = safeInput($datas["ownerid"]);
...
function addPaymentWithdraw($datas)
{
    ...
    } else {//agaciro withdraw
        $userInfo = array("wthac_owner_id" => 0, "wthac_owner_type" => $datas['withdrawertype'], "wthac_company" => $datas['company'], "wthac_name" => $datas['accname'], "wthac_number" => $datas['accnumber']);
    }
    $amount = safeInput($datas["amount"]);
    $date = date("Y-m-d H:i");
    ...
    if ($cnt == 0) {
        $qy = $conn->prepare("INSERT INTO withdraw_history VALUES(:wthid,:withdrawerid,:withdrawertype,:company,:accname,:accnbr,:amountgiven,:amountreq,:reason,:status,:approvedby,:approvaldate,:delstatus,:delreason,:regdate)");
        $qy->execute(array("wthid" => '', "withdrawerid" => $withdrawerid, "withdrawertype" => $datas['withdrawertype'], "company" => $userInfo['wthac_company'], "accname" => $userInfo['wthac_name'], "accnbr" => $userInfo['wthac_number'], "amountgiven" => 0, "amountreq" => $amount, "reason" => "", "status" => 'pending', "approvedby" => 0, "approvaldate" => "0000-00-00", "delstatus" => 0, "delreason" => '', "regdate" => $date));
        //  echo json_encode($qy->errorInfo());
        if ($qy) {
...
function getPaymentWithdraw($datas)
{
    ...
    if (!isset($datas['id'])) {
        ...
        if (isset($datas['status'])) {
            if ($datas['status'] == 'all') {
                $additionalWhere .= " AND withdraw_history.wth_status!='0'";
            } else {
            ...
            if ($datas['cate'] == 'report') {
                ...
                $additionalWhere .= " AND withdraw_history.wth_status='pending'";
            }
        }
...
function declinePaymentWithdraw($datas)
{
    global $conn;
    $feed = "";
    $qyx = $conn->prepare("SELECT * FROM withdraw_history WHERE wth_id=:wthid AND wth_status=:status");
    $qyx->execute(array("wthid" => $datas['id'], "status" => 'pending'));
    if ($qyx->rowCount() > 0) {
        $payInfo = $qyx->fetchAll(PDO::FETCH_ASSOC);
//get payer informations for Notifications
        switch ($payInfo['wth_withdrawer_type']) {
            case 'Commissioner':
                $payerInfo = getUsers(array("id" => $payInfo['wth_withdrawer_type']))['users'][0];
                $phone = $payerInfo['phone'];
                $email = $payerInfo['email'];
                break;
            case 'Postoffice':
                $payerInfo = getPostoffices(array("id" => $payInfo['wth_withdrawer_id']))['postoffices'][0];
                $phone = $payerInfo['phone'];
                $email = $payerInfo['email'];
                break;
            default:
        ...
        $qy = $conn->prepare("UPDATE withdraw_history SET wth_status=:status,wth_reason=:reason,wth_approvedby=:sessid,wth_approval_date=:dates WHERE  wth_id=:pmthid");
        $qy->execute(array("pmthid" => $datas['id'], "reason" => $datas['reason'], "status" => 'declined', "sessid" => $datas['sessid'], "dates" => date("Y-m-d H:i:s")));
        //echo json_encode($qy->errorInfo());exit;
        $count = $qy->rowCount();
        if ($qy) {
            //Notify Payer either by SMS or Email
            $message = "Your withdraw request sent on " . $payInfo['regdate'] . " with amount of " . $payInfo['wth_amount_requested'] . " RWF has been declined <br> Reason:" . $datas['reason'] . "<br> Today " . $date;
            // $response= sendEmail(array("from"=>'noreply@bfg.rw',"fromName"=>'IGISUBIZO System',"to"=>$email,"replyTo"=>'noreply',"subject"=>"Withdraw Request Declined","message"=>$message));
            //Using SMS
            //$response=sendSMS(array("to"=>$phone,"sender"=>'IGISUBIZO',"message"=>$message));
            $feed = "ok";
        } else {
            $feed = "fail";
        }
...
function addIssue($datas)
{
    ...
    $qy = $conn->prepare("INSERT INTO issues VALUES(:id,:ownerid,:ownertype,:title,:status,:delstatus,:delreason,:regdate)");
    ...
    if ($qy) {
        //save issues Message
        $feed = $conn->lastInsertId();
        addIssueChat(array("issid" => $feed, "fromid" => $ownerid, "fromtype" => $ownertype, "toid" => 0, "totype" => 'System', "message" => $datas['message']));
        //auto reply
        addIssueChat(array("issid" => $feed, "fromid" => "0", "fromtype" => "System", "toid" => $ownerid, "totype" => $ownertype, "message" => "Thank you for Sending your query,we'll review it,you'll be answered early.."));
...
function getIssue($datas)
{
    ...
    if ($issuesid == null) {
        ...
        if (isset($datas['ownerid']) && isset($datas['ownertype'])) {
            $datas['toid'] = $datas['ownerid'];
            $datas['totype'] = $datas['ownertype'];
            $additionalWhere = " AND issues.iss_owner_id='" . $datas['ownerid'] . "' AND issues.iss_owner_type='" . $datas['ownertype'] . "'";
        }
        $qy = $conn->prepare("SELECT issues.*,(CASE WHEN issues.iss_owner_type='Commissioner' THEN users.names ELSE postoffices.name END) AS iss_owner,count(issues_chat.isc_id) AS total_unread  FROM issues  LEFT JOIN users ON issues.iss_owner_type='Commissioner' AND  users.uid=issues.iss_owner_id AND users.delete_status=:delstatus LEFT JOIN postoffices ON postoffices.postoff_id=issues.iss_owner_id AND postoffices.delete_status=:delstatus LEFT JOIN issues_chat ON issues_chat.isc_issueid=issues.iss_id WHERE issues.delete_status=:delstatus" . $additionalWhere . " GROUP BY issues.iss_id");
        $qy->execute(array("delstatus" => 0));
//echo json_encode($qy->errorInfo());
    } else {
    ...
    for ($i = 0; $i < count($data); $i++) {//get new Issues chat
        $data[$i]['newchat'] = getIssuesChatSent(array("toid" => $datas['toid'], "totype" => $datas['totype'], "issid" => $data[$i]['iss_id'], "currstatus" => 'sent', "curstatus" => 'delivered'));
        $data[$i]['last_msg'] = getLastMsg(array("issid" => $data[$i]['iss_id']));
    }
...
function getLastMsg($datas)
{
    global $conn;
    $data = null;
    $qy = $conn->prepare("SELECT * FROM issues_chat WHERE issues_chat.isc_issueid=:issid AND issues_chat.delete_status=:delstatus ORDER BY isc_id DESC LIMIT 1");
    $qy->execute(array("issid" => $datas['issid'], "delstatus" => 0));
    $count = $qy->rowCount();
    $data = $qy->fetchAll(PDO::FETCH_ASSOC);
    return $data[0]['isc_message'];
}

function updateIssue($datas)
{
    $data = null;
    global $conn;
    $ownerid = safeInput($datas['ownerid']);
...
function addIssueChat($datas)
{
    ...
    $qy = $conn->prepare("INSERT INTO issues_chat VALUES(:id,:issid,:fromid,:fromtype,:message,:toid,:totype,:status,:delstatus,:delreason,:regdate)");
...
function getIssueChat($datas)
{
    ...
    if ($issues_chatid == null) {
        ...
        $qy = $conn->prepare("SELECT issues_chat.*,issues.iss_title,issues.iss_owner_id,issues.iss_owner_type,(CASE WHEN issues.iss_owner_type='Commissioner' THEN users.names ELSE postoffices.name END) AS iss_owner FROM issues_chat INNER JOIN issues ON issues.iss_id=issues_chat.isc_issueid AND issues.delete_status=:delstatus LEFT JOIN users ON issues.iss_owner_type='Commissioner' AND  users.uid=issues.iss_owner_id AND users.delete_status=:delstatus LEFT JOIN postoffices ON postoffices.postoff_id=issues.iss_owner_id AND postoffices.delete_status=:delstatus
                WHERE issues_chat.isc_issueid=:issid AND issues_chat.delete_status=:delstatus" . $additionalWhere);
        $qy->execute(array("issid" => $datas['issid'], "delstatus" => 0));

    } else {
        $qy = $conn->prepare("SELECT issues_chat.* FROM issues_chat  WHERE issues_chat.delete_status=:delstatus AND issues_chat.isc_id=:iscid");
...
function updateIssueChat($datas)
{
    $data = null;
    global $conn;
    $fromid = safeInput($datas['fromid']);
...
function makeAllSeenIssueChat()
{
    global $conn;
    $qy = $conn->prepare("UPDATE issues_chat SET isc_status=:status
      WHERE isc_issueid=:issueid AND isc_receiver_id=:toid AND isc_receiver_type=:totype");
...
function updateAdminInfo($datas)
{
    global $conn;
    $feed = null;
...
#===================USERS OPERATION==============
function getLostsByRange($cate, $range, $type, $status, $postoff)
{//get Losts Report by Range of Date
    global $conn;
    $data = null;
    $prepWhere = null;
    $start = null;
...
function getDistrictByProvince($provid)
{
    global $conn;
...
function getSectorsByDistrictName($distname)
{
    global $conn;
...
function wholeGeoLocRwanda()
{//return whole json data for rwanda Structure
    $data = null;
    global $conn;
...
function arrangeGeoLocRwanda($geoloc)
{//arrange geoloc rwanda to their prefered ID
    $data = null;
    global $conn;
...
function reportByTypeFromStart()
{
    global $conn;
    $typeName = null;
    $typeCount = null;
...
function getAllIncomesPartner()
{
    ...
    $qyAgaciroWth->execute(array("wthtype" => 'Agaciro', "status" => 'approved'));
    $stmtAgaciroWth = $qyAgaciroWth->fetchAll(PDO::FETCH_ASSOC)[0];

    $total_agaciro_income = $stmtAgaciroReg['total_cit_agaciro'] + $stmtAgaciroLost['total_losts_agaciro'];
    $total_agaciro_balance = $stmtAgaciroReg['total_cit_agaciro'] + $stmtAgaciroLost['total_losts_agaciro'] - $stmtAgaciroWth['total_wth_agaciro'];
...
{
    global $lastPathArr, $lastPath, $server, $client;
    switch ($server->getRequestMethod()) {
        case 'GET':
            $urlData = json_encode($_GET);
            break;
        case'POST':
            $urlData = json_encode($_POST);
            break;
            break;
        default:
            $urlData = json_encode($_GET);
    }
    if ($lastPath == 'igisubizo') {
        $file = "Sysfiles/APPRequest.hd";
    } else {
        $file = "../Sysfiles/APPRequest.hd";
    }
    if (file_exists($file)) {
        $fop0 = fopen($file, "r");
        $data = json_decode(fgets($fop0), TRUE);
        fclose($fop0);
    } else {
        $data = array("request" => array());
        $fop0 = fopen($file, "w");
        fwrite($fop0, json_encode($data));
        fclose($fop0);
    }
    $fop = fopen($file, "w");
    $request = array("date" => date("Y-m-d H:i:s"), "ip" => $client->getIP(), "port" => $server->getPort(), "url" => $server->getURL(), "method" => $server->getRequestMethod(), "data" => $urlData, "type" => $client->getRequestType(), "os" => $client->getOS(), "user_agent" => $server->getUserAgent());
    if ($data == '' || $data == null) {
        $datas = json_encode(array("requests" => array()));
    } else {
        if (count($data['requests']) == 0) {
            $datas = json_encode(array("requests" => array($request)));
        } else {
            $data['requests'][count($data['requests'])] = $request;
...
function readRequests()
{
    $lastPathArr = explode("\\", getcwd());
    $lastPath = $lastPathArr[count($lastPathArr) - 1];
    $method = "$_" . $client->getRequestMethod();

    if ($lastPath == 'igisubizo') {
        $file = "Sysfiles/APPRequest.json";
    } else {
        $file = "../Sysfiles/APPRequest.json";
    }
    if (file_exists($file)) {
        $fop = fopen($file, "a");
        $data = json_decode(fgets($fop), TRUE);
        fclose($fop);
    }
    return $data;
}
...
function checkSessionCookie($category)
{
    ...
    switch ($category) {
        ...
        case'company':
            ...
            break;
        default:
            echo "Session && Cookie category doesn't exist";
    }
}

function cashSeparator($amount, $sep, $interval)
{
    $arr = "";
    $i = strlen($amount);
    if (strlen($amount) <= $interval) {
        $arr = $amount;
    } else {//amount is more than interval
        while ($i > $interval) {
            if ($i == strlen($amount)) {
                $arr = substr($amount, $i - $interval, $interval);
            } else {
                $arr = substr($amount, $i - $interval, $interval) . $sep . $arr;
            }
            $i -= $interval;
        }
        if ($i <= $interval) {
            $arr = substr($amount, 0, $i) . $sep . $arr;
        }
    }
    return $arr;
}
