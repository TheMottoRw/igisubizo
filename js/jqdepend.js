...
clickController("enable","#btnupduser");
		$("#updUserResponse").html("<font color='blue'>Some Information allready Exist to other user Must be different</font> ");
		}else{
clickController("enable","#btnupduser");
...
var postoffice="";
         ...
         postoffice+="<tr>"
             +"<td>"+ obj.losts[i].owner +"</td>"
			  ...
			  +"<td>"+ obj.losts[i].pay_amount+" RWF" +"</td>"
 ...
 var postoffice="";
         ...
         postoffice+="<tr>"
           ...
           +"<td style='text-align:center;position:inherit;' class='infostester'><li class='dropdown' style='list-style-type:none'><a href='#'id='dropBtnTester"+i+"' class='btn btn-primary glyphicon glyphicon-full-screen dropdown-toggle' data-toggle='dropdown'>More <i class='fa fa-caret-down'></i></a>"
               +"</a>"
                 +"<ul id='dropMenus"+i+"' class='dropdown-menu text-white' role='menu' aria-labelledby='dropBtn"+i+"'>"
              +"<li><div class='umoreInfo' style='padding-left:5px;padding-right:5px;'> Balance:"+(loadedpostoffice[i].usr_amount!=null?cashSeparator(loadedpostoffice[i].usr_amount.toString(),",",3):0)+" RWF <br> Paid:"+cashSeparator(loadedpostoffice[i].paid_amount.toString(),",",3)+" RWF<br> Remain:"+cashSeparator(loadedpostoffice[i].usr_remain_amount.toString(),",",3)+" RWF<br> Address:"+loadedpostoffice[i].address+"</div></li>"
              +"</div></ul></li></td>"
             ...
             +"<td>"+ obj.citizens[i].cit_names +"</td>"
              +"<td>"+ obj.citizens[i].cit_nid+"</td>"
               ...
               +"<td>"+(obj.citizens[i].reg_amount==0?'No':'Yes')+"</td>"
                +"<td>"+ obj.citizens[i].regdate.substring(0,10)+"</td>"
...
ajax("ajax/postoffices",{"cate":"update","id":$("#postid").val(),"name":$("#updpostName").val(),"representative":$("#updrepresentativeName").val(),"phone":$("#updpostPhone").val(),"provid":$("#updprovince").val(),"district":$("#upddistrict").val(),"sector":$("#updsector").val(),"cell":$("#updpostCell").val(),"address":$("#updaddress").val()},"GET","text",function(res){
		...
		$("#updPostOfficeResponse").html("<font color='blue'>Some Unique Information exist to other PostOffice,Must be different</font>");
		}else{
...
switch(cate){
	...
	case 'setCombo':
	setComboTypes(res,"#loststype");break;
	case 'setComboByItems':
	setComboTypes(res,"#itemsByType");break;
	default:
	}			
}else{
	$("#loadedtype").html("");
...
function setEditTypes(data){
	...
	$("#updmaxItem").val(data.types[0].max_items);
}
...
function updateTypes(){
 ...
 ajax("ajax/types",{"cate":"update","id":$("#typeid").val(),"doctype":$("#updType").val(),"max":$("#updmaxItem").val()},"GET","text",function(res){
		...
		$("#updateTypeResponse").html("<font color='blue'>Some Unique Information Exist to Other Document Type</font>");
		}else{
...
switch(cate){
  ...
  case 'setCombo':
  setComboWithdrawCompany(res,"#wthCompany",reference);break;
  default:
  }     
}else{
...
function updateWithdrawCompany(){
 ...
 ajax("ajax/wthcompany",{"cate":"update","id":$("#wthcompanyid").val(),"accr":$("#updWthcAccr").val(),"name":$("#updWthcName").val()},"GET","text",function(res){
    ...
    $("#updateWithdrawCompanyResponse").html("<font color='blue'>Sorry...! Some Unique Information Exist to Other Withdraw Company</font>");
    }else{
     ...
     for(var i=0;i<loadedpayment.payments.length;i++){  
         payinfo+="<tr>"
             ...
             +"<td>"+ loadedpayment.payments[i].pay_name +"</td>"
              ...
              +"<td>"+ cashSeparator(loadedpayment.payments[i].pay_amount.toString(),",",3) +" RWF</td>"
     ...
     for(var i=0;i<loadedpayment.payhistory.length;i++){  
         payinfo+="<tr>"
             ...
             +"<td>"+ (loadedpayment.payhistory[i].payer_names==null?'None':loadedpayment.payhistory[i].payer_names)+"</td>"
              ...
              +"<td>"+ cashSeparator(loadedpayment.payhistory[i].pmth_amount.toString(),",",3) +" RWF</td>"
              +"<td>"+ loadedpayment.payhistory[i].pmth_status+"</td>"
              +"<td>"+ loadedpayment.payhistory[i].regdate.substring(0,10)+"</td>"
              +"<td class='loadedapproval' style='text-align:center;'><a href='#' class='btn btn-warning edit fa fa-check-square-o' data-toggle='modal' data-target='#approvePaymentModal' "+(loadedpayment.payhistory[i].pmth_status!='pending'?'disabled':"onclick='loadPaymentsHistoryById(\"approve\","+loadedpayment.payhistory[i].pmth_id+")'")+">Approve</a>"
              +"<a href='#' class='btn btn-danger edit fa fa-close' data-toggle='modal' data-target='#declinePaymentModal' "+(loadedpayment.payhistory[i].pmth_status!='pending'?'disabled':"onclick='loadPaymentsHistoryById(\"decline\","+loadedpayment.payhistory[i].pmth_id+")'")+">Decline</a></td>"
              +"</tr>";
      }
...
$("#payhistid").val(res.payhistory[0].pmth_id);
$("#ampaid").val(res.payhistory[0].pmth_amount);
  }
  if(cate=='edit'){
...
function declinePaymentsHistory(){
 setLoaders({elem:'declinePaymentResponse',elemtype:'container',msg:'Declining Payment History...'});
 ajax("ajax/payhist",{"cate":"decline",sessid:$("#sessid").val(),"id":$("#payhistid").val(),"reason":$("#declinePmthReason").val()},"GET","text",function(res){
    if(res=="ok"){
      loadPaymentsHistory("setContent");
    $("#declinePaymentResponse").html("<font color='green'>Payment Declined Success</font>");
    }else if(res=='notexist'){
    $("#declinePaymentResponse").html("<font color='blue'>Payment Allready Declined</font> ");
    }else{
    $("#declinePaymentResponse").html("<font color='red'>Failed to Decline Payment</font> ");
    }
clearMsg("#declinePaymentResponse");
  });
}
...
clickController("disable","#btnSaveWth");
  ajax("ajax/withdrawhist",{"cate":"register",sessid:$("#sessid").val(),"company":$("#wthCompany").val(),"accname":$("#accName").val(),"accnumber":$("#accNumber").val(),"amount":$("#reqAmount").val(),"withdrawerid":"0","withdrawertype":"Agaciro"},"GET","text",function(res){
    if(res=="ok"){
clickController("enable","#btnSaveWth");
      loadWithdrawsHistory("setContent");
      $("#reqAmount").val("");$("#accName").val("");$("#accNumber").val("");
...
var wthinfo="";
     for(var i=0;i<loadedwthment.withdraws.length;i++){  
         wthinfo+="<tr>"
             +"<td>"+ (i+1) +"</td>"
             +"<td>"+ (loadedwthment.withdraws[i].wth_withdrawer_type!='Agaciro'?loadedwthment.withdraws[i].withdrawer_names:loadedwthment.withdraws[i].wth_withdrawer_type)+"</td>"
              ...
              +"<td>"+ cashSeparator(loadedwthment.withdraws[i].wth_amount_requested.toString(),",",3) +" RWF</td>"
              +"<td>"+ cashSeparator(loadedwthment.withdraws[i].wth_amount_given.toString(),",",3)+" RWF</td>"
              +"<td>"+ loadedwthment.withdraws[i].wth_status+"</td>"
              +"<td>"+ loadedwthment.withdraws[i].regdate.substring(0,10)+"</td>"
              +"<td class='loadedwthmodif' style='text-align:center;'><a href='#' class='btn btn-warning edit fa fa-check-square-o' data-toggle='modal' data-target='#approveWithdrawModal' "+(loadedwthment.withdraws[i].wth_status!='pending'?'disabled':"onclick='loadWithdrawsHistoryById(\"approve\","+loadedwthment.withdraws[i].wth_id+")'")+">Approve</a>"
              +"<a href='#' class='btn btn-danger edit fa fa-close' data-toggle='modal' data-target='#declineWithdrawModal' "+(loadedwthment.withdraws[i].wth_status!='pending'?'disabled':"onclick='loadWithdrawsHistoryById(\"decline\","+loadedwthment.withdraws[i].wth_id+")'")+">Decline</a></td>"
              +"</tr>";
      }
...
$("#given").val(res.withdraws[0].wth_amount_given);
   ...
   if(cate=='decline'){
$("#withdrawhistid").val(res.withdraws[0].wth_id);
$("#amrequested").val(res.withdraws[0].wth_amount_requested);
  }
...
function declineWithdrawsHistory(){
clickController("disable","#btnDeclineWithdraw");
  setLoaders({elem:'declineWithdrawResponse',elemtype:'container',msg:'Declining Withdraw History...'});
 ajax("ajax/withdrawhist",{"cate":"decline",sessid:$("#sessid").val(),"id":$("#withdrawhistid").val(),"reason":$("#declineWthReason").val()},"GET","text",function(res){
    if(res=="ok"){
clickController("enable","#btnDeclineWithdraw");
      loadWithdrawsHistory("setContent");
    $("#declineWithdrawResponse").html("<font color='green'>Withdraw Declined Success</font>");
    }else if(res=='notexist'){
clickController("enable","#btnDeclineWithdraw");
    $("#declineWithdrawResponse").html("<font color='blue'>Withdraw Allready Declined</font> ");
    }else{
clickController("enable","#btnDeclineWithdraw");
    $("#declineWithdrawResponse").html("<font color='red'>Failed to Decline Withdraw</font> ");
    }
clearMsg("#declineWithdrawResponse");
  });
}
...
ajax("ajax/paymodes",{"cate":"update","sessid":$("#sessid").val(),"id":$("#paymodeid").val(),"modename":$("#updpaymodename").val(),"company":$("#updcompany").val(),"accname":$("#updaccname").val(),"accnumber":$("#updaccnumber").val()},"POST","text",function(res){
		...
		$("#updPaymodeResponse").html("<font color='blue'>Some Unique Information Exist to Other Paymode</font>");
		}else{
...
clickController("disable","#btnSaveIssue");
    setLoaders({elem:'regIssueResponse',elemissue:'container',msg:'Saving Data...'});
    ajax("ajax/issues",{"cate":"register",sessid:$("#sessid").val(),"docissue":$("#issue").val(),"max":$("#maxItem").val()},"GET","text",function(res){
        if(res=="ok"){
clickController("enable","#btnSaveIssue");
            loadIssues("setContent");
            $("#issue").val("");$("maxItem").val("");
...
clearMsg("#regIssueResponse");
    });
}
function loadIssues(cate){
    if(cate=='setContent'){
    setLoaders({elem:'tblIssues',elemissue:'table',msg:'Loading Data...'});
}
    ajax("ajax/issues",{"cate":"load",toid:0,totype:'System'},"GET","json",function(res){
if(res.issues!=null){    
switch(cate){
    case'setContent':
...
if(loadedissue.issues[i].iss_id!=null){       
         issues+="<tr>"
             ...
             +"<td>"+ loadedissue.issues[i].iss_status+"</td>"
              ...
              +"<td style='text-align:center;position:inherit;' class='cflmore'><li class='dropdown' style='list-style-type:none'><a href='#'id='dropBtn"+i+"' class='btn btn-"+((loadedissue.issues[i].newchat!=0)?'success':'info')+" glyphicon glyphicon-full-screen dropdown-toggle' data-toggle='dropdown'>Actions <i class='fa fa-caret-down'></i></a>"
               +"</a>"
                 +"<ul id='dropMenus"+i+"' class='dropdown-menu text-white' role='menu' aria-labelledby='dropBtn"+i+"'>"
              +"<li><a href='#' onclick='loadIssuesChat(\"setContent\","+loadedissue.issues[i].iss_id+")' class='btn btn-success view glyphicon' data-toggle='modal' data-target='#issueChatModal'>Reply</a></li>"
...
$("#loadedIssues").html("");
            $("#loadedIssues").append(issues);
            jslive.pagingTable({id:'#tblIssues',shows:25,active:0});
    }
function loadIssuesById(cate,id){
    ajax("ajax/issues",{"cate":"loadbyid","id":id},"GET","json",function(res){
    if(cate=='reply'){
...
function searchIssues(){
    ajax("ajax/issues",{"cate":"search","key":$("#keyIssue").val()},"GET","json",function(res){
if(res.issues!=null){
    setLoadedIssues(res);
    }else{
...
function updateIssues(){
 ...
 ajax("ajax/issues",{"cate":"update","id":$("#issueid").val(),"docissue":$("#updIssue").val(),"max":$("#updmaxItem").val()},"GET","text",function(res){
        if(res=="ok"){
            loadIssues("setContent");
...
function deleteIssues(){
    ajax("ajax/issues",{"cate":"delete","id":$("#deleteid").val(),"reason":$("#delReason").val()},"GET","text",function(res){
    //  alert(res);
        if(res=="ok"){
        loadIssues("setContent");
        $("#delReason").val("");
...
clickController("disable","#btnSaveIssue");
     ajax("ajax/issueschat",{"cate":"register",sessid:$("#sessid").val(),"issid":$("#iscissueid").val(),"fromtype":'System',"toid":$("#toid").val(),"totype":$("#totype").val(),"message":$("#issueschat").val()},"GET","text",function(res){
        if(res=="ok"){
        	$("#issueschat").val("");
clickController("enable","#btnSaveIssue");
...
function loadIssuesChat(cate,issid){
    ...
    if(cate=='setContent'){
    setLoaders({elem:'chatContainer',elemissue:'container',msg:'Loading Data...'});
}
    ajax("ajax/issueschat",{"cate":"load","issid":issid},"GET","json",function(res){
if(res.issues_chat.length!=0){    
switch(cate){
    case'setContent':
    setLoadedIssuesChat(res);
changeIssuesStatus(res.issues_chat[0].isc_issueid,'sent','progress');
changeAllChatStatus(res.issues_chat[0].isc_issueid,'delivered','seen');
    break;
    default:
    }           
}else{
...
chatMsg+="<div class='"+(loadedissue.issues_chat[i].isc_from_type=='System'?'iscSent':'iscReceived')+"'>"+loadedissue.issues_chat[i].isc_message+"<br>"
                +"<span class='sentdate'>"+loadedissue.issues_chat[i].regdate.substring(0,16)+"</span><span class='status'>"+(loadedissue.issues_chat[i].isc_from_type=='System'?loadedissue.issues_chat[i].isc_status:'')+"</span></div>";
            }
$("#chatContainer").html("");
...
function loadIssuesChatById(cate,id){
    ajax("ajax/issueschat",{"cate":"loadbyid","id":id},"GET","json",function(res){
    if(cate=='edit'){
...
function searchIssuesChat(){
    ajax("ajax/issueschat",{"cate":"search","key":$("#keyIssue").val()},"GET","json",function(res){
...
function updateIssuesChat(){
    setLoaders({elem:'updateIssueResponse',elemissue:'container',msg:'Updating Data...'});
 ajax("ajax/issueschat",{"cate":"update","id":$("#issueid").val(),"docissue":$("#updIssue").val(),"max":$("#updmaxItem").val()},"GET","text",function(res){
        if(res=="ok"){
...
function deleteIssuesChat(){
    ajax("ajax/issueschat",{"cate":"delete","id":$("#deleteid").val(),"reason":$("#delReason").val()},"GET","text",function(res){
    //  alert(res);
        if(res=="ok"){
        loadIssuesChat("setContent");
        $("#delReason").val("");
...
ajax("ajax/notifiers",{"cate":"newissues",currstatus:'sent'},"GET","text",function(res){
...
ajax("ajax/notifiers",{"cate":"changeissstatus",issid:issid,status:status,currstatus:currstatus},"GET","text",function(res){

})
}
function changeAllIssuesStatus(status,currstatus){
ajax("ajax/notifiers",{"cate":"changeissuesstatus",status:status,currstatus:currstatus},"GET","text",function(res){

})
}
function changeAllChatStatus(issid,currstatus,status){
ajax("ajax/notifiers",{"cate":"changeiscstatus",issid:issid,status:status,currstatus:currstatus,toid:0,totype:"System"},"GET","text",function(res){
...
$("#postincome").html((res.dashheader.postoffice.income==null?0:cashSeparator(res.dashheader.postoffice.income.toString(),",",3))+ 'RWF');
$("#postbalance").html((res.dashheader.postoffice.balance==null?0:cashSeparator(res.dashheader.postoffice.balance.toString(),",",3))+ 'RWF');
$("#commincome").html((res.dashheader.commissioner.income==null?0:cashSeparator(res.dashheader.commissioner.income.toString(),",",3))+ 'RWF');
$("#commbalance").html((res.dashheader.commissioner.balance==null?0:cashSeparator(res.dashheader.commissioner.balance.toString(),",",3))+ 'RWF');
$("#agaciroincome").html((res.dashheader.agaciro.income==null?0:cashSeparator(res.dashheader.agaciro.income.toString(),",",3))+ 'RWF');
$("#agacirobalance").html((res.dashheader.agaciro.balance==null?0:cashSeparator(res.dashheader.agaciro.balance.toString(),",",3))+ 'RWF');
		}
	});
}
...
//===========OPTIONAL METHOD=======================
function cashSeparator(number,sep,interval){
var nbrArr="",nbrCount=number.length;
//console.log(number.substring(2,4));
if(nbrCount<=interval){
	nbrArr=number;
}else{//has more items than interval
for(var i=Math.ceil(nbrCount/interval);i>0;i--){
	if(i==Math.ceil(nbrCount/interval)){
	nbrArr=number.substring(nbrCount-interval);	
	number=number.substring(0,nbrCount-interval);
}else{
if(number.length>interval){
nbrArr=number.substring(number.length-interval)+sep+nbrArr;
number=number.substring(0,number.length-interval);
}else{
nbrArr=number+sep+nbrArr;
}
}
}
}
